...
"""
ici on vas selectionner la vigne l'outil et creer le travail qui sera effectue a la vigne
...
class WindowTravail:
    def __init__(self, window_m):
        ...
        self.libelle_general = self.font_g.render("TRAVAIL ROUTINE NON IMPLANTE", True, config.WHITE, config.BLACK)
    ...
    def gestion(self):
        while True:
            titre = "** EasyVine TRAVAIL**  "  # lat: " + format(self.latitude, '.7f') + "   long: " + format(self.longitude, '.7f') + "  track: " + format(self.track,'.2f') + "  altitude: " + format(self.altitude, '.4f')
