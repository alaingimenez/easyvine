...
class WindowCreat:
    def __init__(self, window_m):
        ...
        self.libelle_general = self.font_g.render("CREAT ROUTINE NON IMPLANTE", True, config.WHITE, config.BLACK)
    ...
    def gestion(self):
        while True:
            titre = "** EasyVine CREAT **  "  # lat: " + format(self.latitude, '.7f') + "   long: " + format(self.longitude, '.7f') + "  track: " + format(self.track,'.2f') + "  altitude: " + format(self.altitude, '.4f')
